...
final model achieves 2.32\,mm test MAE at a 98\,ms prediction horizon,
outperforming the persistence baseline (4.36\,mm) by 47\% and the deployed EMA
pipeline (7.20\,mm) by 68\%. The model is exported to TorchScript and runs
...
3{,}255 validation, and 3{,}257 test windows). Training converged without
overfitting (Figure~\ref{fig:loss}); validation loss ending below training
loss indicates the dataset is adequate for this model capacity (the
validation slices happen to contain somewhat calmer motion than the training
slices, a side effect of the chronological split).
...
\begin{itemize}
  ...
  \item \textbf{Single test distribution.} The test split is the chronological
        tail of each segment; metrics carry the variance of a $\sim$1.8-minute
        test set.
